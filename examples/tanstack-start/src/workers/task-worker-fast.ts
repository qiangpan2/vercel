...
async function recordEvent(jobId: string, event: TaskEventPayload) {
	...
	await redis.xadd(streamKey(jobId), 'MAXLEN', '~', 200, '*', 'event', event.event, 'payload', payload)
}
...
const worker = new Worker<TaskPayload>(
	...
	async (job) => {
		...
		const onEvent = async (event: TaskEventPayload) => {
			...
			recordAuditEvent(job.id as string, QUEUE_NAMES.fast, job.name, enriched)
		}
...
worker.on('failed', async (job, error) => {
	console.error('[fast-worker] failed', job?.id, error)
...
worker.on('error', (error) => {
	console.error('[fast-worker] worker error', error)
})

console.log('🚀 Fast worker started')
